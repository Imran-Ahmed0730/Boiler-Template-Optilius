...
@section('content')
    <div class="app-content-header"> <!--begin::Container-->
        <div class="container-fluid"> <!--begin::Row-->
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="mb-0">View Support Tickets</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center">
                            <h3 class="card-title mb-0">View Support Tickets</h3>
                        </div>
                        <!-- /.card-header -->
                            ...
                            <table class="table table-bordered table-hover" id="datatable">
                                ...
                                @foreach($items as $key => $item)
                                    <tr class="align-middle">
                                        ...
                                        <td><span class="p-2 badge text-bg-{{$item->status == 1 ? 'success': 'danger'}}">{{$item->status == 1 ? 'Active':'Closed'}}</span></td>
                                        <td>
                                            <div class="d-flex">
                                                @can('Support Chat View')
                                                <a href="{{route('admin.support.chat', $item->id)}}" class="btn btn-primary me-2" title="Chat"><i class="fa fa-comments"></i></a>
                                                @endcan
                                                @if($item->status == 1)
                                                    @can('Support Close')
                                                        <a href="{{route('admin.support.close', $item->id)}}" title="Close" class="btn btn-danger me-2" id="close-button"
                                                           data-url="{{ route('admin.support.close', $item->id) }}"><i class="fa fa-lock"></i></a>
                                                    @endcan
                                                @else
                                                    @can('Support Open')
                                                        <a href="{{route('admin.support.open', $item->id)}}" title="Open" class="btn btn-success me-2">
                        ...
                        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <form action="{{route('admin.support.assign')}}" method="post">
                                        @csrf
