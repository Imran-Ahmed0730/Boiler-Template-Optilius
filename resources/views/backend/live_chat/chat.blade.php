                ...
                <div class="col-md-12">
                    <div class="card card-primary card-outline mb-4"> <!--begin::Header-->
                        <div class="card-header d-flex align-items-center">
                            <div class="card-title">Chat With: <span>{{$item->user->name}}</span></div>
                        </div> <!--end::Header--> <!--begin::Form-->
                            <div class="card-body">
                                <div class="chat-body d-flex flex-column">
                                    @foreach ($item->chat as $chat)
                                        @if ($chat->sent_by != 1 && $item->sent_by != 4)
                            ...
                            @can('Live Chat')
                                ...
                                <div class="card-footer">
                                    <form id="chatForm" action="{{ route('admin.live-chat.chat.message.send') }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="chat-footer d-flex align-items-center mt-3">
                                            <!-- Add Attachment Button -->
                                            <button type="button" class="d-none attachment-btn">+</button>
                                            ...
                                            <input type="hidden" name="chat_user_id" value="{{ $item->id }}">
...
@push('js')
    ...
    <script>
        ...
        var channel = pusher.subscribe('chat'+{{$item->id}});

        // Bind to the event
        channel.bind('admin.message', function(data) { // Updated event name to 'user-message'
            ...
            $('#chatForm').on('submit', function (event) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#chatForm')[0].reset(); // Reset form fields
